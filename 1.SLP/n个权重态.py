...
for epoch in range(epoch_num):
    ...
    if current_epoch in [10, 50, 100, 150, 200]:
        dist_of_num = count_num(parameters)
        text_save('./TEST/' + G_num + '/epoch=%d' %current_epoch, dist_of_num)
        heat_map = np.zeros(100).reshape(10, 10)
        for i in range(10):
            for j in range(10):
                heat_map[i][j] = dist_of_num[i][j]
        print(heat_map)
        plt.imshow(heat_map, cmap='inferno')
        plt.savefig('./TEST/' + G_num + '/热度图 epoch=%d Gn=%d.png' %(current_epoch, int(G_num)), dpi=720)
        plt.show()
...
plt.savefig('./TEST/' + G_num + '/准确率 epoch=%d Gn=%d.png' %(current_epoch, int(G_num)), dpi=720)
plt.show()

train_filename = './TEST/' + G_num + '/train_accu Gn=%d epoch=%d batch=%d learn_rate=%0.2e.txt' % (int(G_num), epoch_num, batch_size, learn_rate)
test_filename = './TEST/' + G_num + '/test_accu Gn=%d epoch=%d batch=%d learn_rate=%0.2e.txt' % (int(G_num), epoch_num, batch_size, learn_rate)
